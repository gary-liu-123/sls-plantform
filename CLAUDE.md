...
| `com.example.photo.EntryWorkOrderFieldsTest` | @Test | — | — | 查询入职工单字段定义（GET `/api/v1/fields?objectApiName=entryWorkOrder`），返回每个字段的 id/label/apiName/fieldTypeName/permissionCode/optionList |
...
| `https://www.udesk.cn/doc/sercive/custom-fields/`（在线 / Udesk 开发者中心 - 自定义字段接口） | 官方在线文档 | 字段类型对照表（`field_type_*` ↔ 中文 ↔ 值格式）、**`GET /v1/fields`**（按 `objectApiName` 拉全部字段元数据）、`POST /v1/fields`（按 `fieldApiNameList` 查部分字段）、`PUT /v1/field`（更新字段）、`POST /v1/fields/options`（创建单选/多选/级联选项值） | 写 `*FieldsTest` 类时的协议来源：决定 URL/参数结构；同时也是排查"某字段是单选还是多选、选项有哪些"的兜底（不依赖本地 md 字段字典） |
...
| `EntryWorkOrderFieldsTest` | `entryWorkOrder` | — | GET `/api/v1/fields` | `objectApiName=entryWorkOrder`（无需 id/手机号，按对象拉全部字段元数据） |
