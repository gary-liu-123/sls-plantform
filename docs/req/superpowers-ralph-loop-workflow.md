...
## 备注：启动脚本要求（写入 ralph-loop 命令）

在前后端首次启动后，需要在项目根目录下新建 `/scripts` 目录，并生成启动脚本 `scripts/start_all.sh`。

**脚本功能要求：**
1. 首先查找并杀死历史占用端口的进程（按端口号精确查杀）
2. 然后重启前后端服务
3. 将启动日志输出到项目根目录下的 `/logs` 目录

**CLAUDE.md 更新要求：**
- 脚本生成后，更新项目 `CLAUDE.md`，增加一条规则：每次需要启动项目时，都使用 `scripts/start_all.sh` 来启动

**在生成 ralph-loop 命令时，请将以上要求作为启动脚本相关步骤写入 prompt 中。**

## 备注：Git 提交规则（写入 ralph-loop 命令）

提交代码时，**禁止提交**以下文件：
- 日志文件（`/logs` 目录、`*.log`）
- 截图文件（`*.png`、`*.jpg`、`*.jpeg`、`*.gif`、`*.bmp`）
- 编译后生成的文件（`target/`、`dist/`、`build/`、`*.class`、`*.jar`）

确保 `.gitignore` 中包含以上规则，ralph-loop 每次 commit 前需检查暂存区不含这些文件。
