...
- 子路径部署能力（如需要）：Vite `base` + React Router `basename` + `VITE_API_BASE_URL`
- **Secrets 管理（Day 1 就规范）**：
  - 根目录 `.env.example`（提交）+ `.env`（gitignore）
  - 后端 `application.yml`（提交，只放配置结构）+ `application-local.yml`（提交，放本地密钥）+ `application-prod.yml`
  - `.gitignore` 起手就加：`.env`, `*.pem`, `*.key`
- **文档目录职责（写死，避免乱）**：
...
**4b. 前端模块 + chrome-devtools MCP 验证**（重要）
...
**4e. ralph-loop 卡 max-iterations 时的恢复 checklist**
...
**4f. 每个模块跑完记一笔到 `docs/superpowers/ralph-log.md`**
格式：
```
## <module> - <date>
...
**4g. CLAUDE.md 维护规则（重要）**
ralph-loop 输出 promise 退出后、commit 前，必须按下面规则同步项目级 `CLAUDE.md`，让下一个会话/下一个 ralph-loop 不用从零摸索。

**该进 CLAUDE.md（每次新会话都需要的"项目说明书"）**
- 启动/停止命令：`scripts/start.sh`、`scripts/stop.sh`、本地端口、健康检查 URL
- 常用命令：`mvn test`、`npm test`、`npm run gen:api`、`mvn flyway:migrate`
- 技术栈核心版本（Spring Boot / React / JDK / Node 等大版本）
- 目录约定：`frontend/` / `backend/` / `docs/requirements/` / `docs/superpowers/...`
- 关键工程约定：Flyway 必须走 migration（禁 `ddl-auto=update`）、API 类型走 codegen、secrets 放 `.env` / `application-local.yml`
- 本地测试账号：用户名 / 密码 / 角色，直接写明文（仅限本地 dev 假数据）
- 主要环境变量名清单（不含值）：`VITE_API_BASE_URL`、`SPRING_PROFILES_ACTIVE` …
- 关键文档地图：design / plans / ralph-log / frontend-verify-checklist 的位置
...
- 框架核心版本升级（Spring Boot / React / JDK / Node 大版本动了）
...
**ralph-loop 内部如何执行此规则**
- 每个模块的 ralph-loop prompt 里都有一步「按 CLAUDE.md 维护规则检查」（见 4a / 4b 模板）
- 检查动作：对照"触发事件"清单，命中任一就编辑 CLAUDE.md 对应章节；没命中就跳过，**不要为了"显得有产出"硬塞内容**
- 修改后 `git diff CLAUDE.md` 自检：每行新增都能对应到本轮真实的代码/脚本变化，不能凭空写"未来计划"
...
## 关键经验（避坑）
1. **Word 必须先按模块拆 md**：整份大文档塞进 prompt 会被截断，模型抓不住重点
...
10. **每个 ralph-loop 退出前必须同步 CLAUDE.md**：按 4g 规则判断是否触发更新，触发就改、不触发就跳过；测试账号直接写进 CLAUDE.md「本地测试账号」章节

## 验证（怎么知道每阶段做对了）
- Phase 1：Word 已转 md 并按模块拆成 `docs/requirements/*.md`；`99-open-questions.md` 全部标 `[resolved]` 或 `[deferred]`，无 `[open]`
...
- Phase 4：每个模块的 ralph-loop 都用真实的 promise 退出（不是被 max-iterations 卡掉）；前端模块都通过 chrome-devtools MCP 浏览器验证；`docs/superpowers/ralph-log.md` 有对应记录；`CLAUDE.md` 与本轮代码改动一致（启动命令、目录约定、测试账号、环境变量都最新）
...
- chrome-devtools MCP：`new_page` / `fill_form` / `click` / `wait_for` / `list_console_messages` / `list_network_requests` / `take_snapshot`
