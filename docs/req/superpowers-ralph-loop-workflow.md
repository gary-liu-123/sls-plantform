...
每个模块的 ralph-loop prompt 里都有一步「按 CLAUDE.md 维护规则检查」（见 4a / 4b 模板）。检查动作：对照"触发事件"清单，命中任一就编辑 CLAUDE.md 对应章节；没命中就跳过，**不要为了"显得有产出"硬塞内容**。修改后 `git diff CLAUDE.md` 自检：每行新增都能对应到本轮真实的代码/脚本变化，不能凭空写"未来计划"。
...
- 技术栈核心版本（Spring Boot / Vue 3 / JDK / Node 等大版本）
